...
@Component({
  selector: 'app-table-tracage-des-donnes',
  templateUrl: './table-tracage-des-donnes.component.html',
  styleUrls: ['./table-tracage-des-donnes.component.css']
})
export class TableTracageDesDonnesComponent implements OnInit {
  ...
  tableGestionProfils = GestionProfilsMetadata.tableTracageDonnes;
  ...
  openDetails(type) {
    ...
    this.router.navigate(['/app/adm/tdd/d/' +type.idLogData]);
  }
  ...
  generateFile(fileType: 'pdf' | 'excel') {
    switch (fileType) {
      case "pdf":
        this.documentExporterService.generatePdf('adm.tdd.table', this.dataSource.data, this.tableGestionProfilsDetails.exportedColumns, "landscape")
        return;
      case "excel":
